...
{
    if(!privateSendPool.IsBlockchainSynced() || ShutdownRequested()) return;

    if(!pwalletMain) return;

    QString strAmountAndRounds;
    QString strAnonymizeRupeeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRupeeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeRupeeAmount = strAnonymizeRupeeAmount.remove(strAnonymizeRupeeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRupeeAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeRupeeAmount*COIN) nMaxToAnonymize = nAnonymizeRupeeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeRupeeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeRupeeAmount));
        strAnonymizeRupeeAmount = strAnonymizeRupeeAmount.remove(strAnonymizeRupeeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRupeeAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
...
void OverviewPage::privateSendStatus()
{
    ...
    // Get PrivateSend Denomination Status
}
...
void OverviewPage::togglePrivateSend(){
    ...
    if(hasMixed.isEmpty()){
        QMessageBox::information(this, tr("PrivateSend"),
                tr("If you don't want to see internal PrivateSend fees/transactions select \"Received By\" as Type on the \"Transactions\" tab."),
                QMessageBox::Ok, QMessageBox::Ok);
