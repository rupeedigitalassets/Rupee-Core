    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Rupee address (e.g. RdgAqg3Cr2RoEdqYk3WsAqecKyf9pRMn4t)"));
#endif
    ...
    if (!settings.contains("bUseInstandSend"))
        settings.setValue("bUseInstandSend", false);

    bool usePrivateSend = settings.value("bUsePrivateSend").toBool();
    bool useInstandSend = settings.value("bUseInstandSend").toBool();

    if(fLiteMode) {
        ui->checkUsePrivateSend->setChecked(false);
        ui->checkUsePrivateSend->setVisible(false);
        ui->checkInstandSend->setVisible(false);
        CoinControlDialog::coinControl->usePrivateSend = false;
        CoinControlDialog::coinControl->useInstandSend = false;
    }
    else{
        ui->checkUsePrivateSend->setChecked(usePrivateSend);
        ui->checkInstandSend->setChecked(useInstandSend);
        CoinControlDialog::coinControl->usePrivateSend = usePrivateSend;
        CoinControlDialog::coinControl->useInstandSend = useInstandSend;
    }

    connect(ui->checkUsePrivateSend, SIGNAL(stateChanged ( int )), this, SLOT(updateDisplayUnit()));
    connect(ui->checkInstandSend, SIGNAL(stateChanged ( int )), this, SLOT(updateInstandSend()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(ui->checkInstandSend->isChecked()) {
        recipients[0].useInstandSend = true;
        strFunds += " ";
        strFunds += tr("and InstandSend");
    } else {
        recipients[0].useInstandSend = false;
    }
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CRupeecoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::updateInstandSend()
{
    QSettings settings;
    settings.setValue("bUseInstandSend", ui->checkInstandSend->isChecked());
    CoinControlDialog::coinControl->useInstandSend = ui->checkInstandSend->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstandSend doesn't support sending values that high yet. Transactions are currently limited to 3000 RUP.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CRupeecoinAddress addr = CRupeecoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Rupee address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
