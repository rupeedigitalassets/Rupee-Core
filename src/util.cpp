...
bool fEnableInstandSend = true;
int nInstandSendDepth = 10;
int nPrivateSendRounds = 2;
int nAnonymizeRupeeAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Rupee
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Rupee
    // Mac: ~/Library/Application Support/Rupee
    // Unix: ~/.Rupee
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RupeeCore";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Rupee.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Rupeed.pid"));
