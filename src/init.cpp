...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Rupee.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Rupeed.pid)") + "\n";
    ...
    strUsage += "  -litemode=<n>          " + _("Disable all PrivateSend and Stealth Messaging related functionality (0-1, default: 0)") + "\n";
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -enableprivatesend=<n>          " + _("Enable use of automated privatesend for funds stored in this wallet (0-1, default: 0)") + "\n";
    strUsage += "  -privatesendrounds=<n>          " + _("Use N separate masternodes to anonymize funds  (2-8, default: 2)") + "\n";
    strUsage += "  -anonymizeRupeeamount=<n> " + _("Keep N Rupee anonymized (default: 0)") + "\n";
    strUsage += "  -liquidityprovider=<n>       " + _("Provide liquidity to PrivateSend by infrequently mixing coins on a continual basis (0-100, default: 0, 1=very frequent, high fees, 100=very infrequent, low fees)") + "\n";

    strUsage += "\n" + _("InstandSend options:") + "\n";
    strUsage += "  -enableinstandsend=<n>    " + _("Enable instandsend, show confirmations for locked transactions (bool, default: true)") + "\n";
    strUsage += "  -instandsenddepth=<n>     " + strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nInstandSendDepth) + "\n"; 
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Rupee is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Rupee is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Rupee version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Rupee server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Rupee") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Rupee to complete") << "\n";
    ...
    nAnonymizeRupeeAmount = GetArg("-anonymizeRupeeamount", 0);
    if(nAnonymizeRupeeAmount > 999999) nAnonymizeRupeeAmount = 999999;
    if(nAnonymizeRupeeAmount < 2) nAnonymizeRupeeAmount = 2;

    fEnableInstandSend = GetBoolArg("-enableinstandsend", fEnableInstandSend);
    nInstandSendDepth = GetArg("-instandsenddepth", nInstandSendDepth);
    nInstandSendDepth = std::min(std::max(nInstandSendDepth, 0), 60);
    ...
    LogPrintf("nInstandSendDepth %d\n", nInstandSendDepth);
    LogPrintf("PrivateSend rounds %d\n", nPrivateSendRounds);
    LogPrintf("Anonymize Rupee Amount %d\n", nAnonymizeRupeeAmount);
