...
const string strMessageMagic = "Rupee Signed Message:\n";
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // ----------- instandsend transaction scanning -----------
...
{
    if(!IsSporkActive(SPORK_2_INSTANDSEND)) return -3;
    if(!fEnableInstandSend) return -1;
    ...
    if(enableIX){
        if (nResult < 10){
            ...
            if(signatures >= INSTANDSEND_SIGNATURES_REQUIRED){
                return nInstandSendDepth+nResult;
            }
        }
    }
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6){
        ...
        if(sigs >= INSTANDSEND_SIGNATURES_REQUIRED){
            return nInstandSendDepth+nResult;
        }
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	nSubsidy >>= (nHeight / 545000); // Halving: 545000 blocks
...
// ----------- instandsend transaction scanning -----------
    ...
    if(IsSporkActive(SPORK_3_INSTANDSEND_BLOCK_FILTERING)){
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if(!IsInitialBlockDownload()){
        ...
        // If we're in LiteMode disable privatesend features without disabling masternodes
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else
    {
        ...
        ProcessMessageInstandSend(pfrom, strCommand, vRecv);
