...
Value privatesend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "privatesend <Rupeeaddress> <amount>\n"
            "Rupeeaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "privatesend <Rupeeaddress> <amount>\n"
            "Rupeeaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in Rupee.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in Rupee.conf\n"
                ...
                "  vote-many    - Vote on a Rupee initiative\n"
                "  vote         - Vote on a Rupee initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CRupeecoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if(!privateSendSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, keyMasternode)){
                printf(" Error upon calling SignMessage for %s\n", mne.getAlias().c_str());
                failed++;
                continue;
            }

            if(!privateSendSigner.VerifyMessage(pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)){
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Rupee initiative (can be additionally filtered, partial match)\n"
