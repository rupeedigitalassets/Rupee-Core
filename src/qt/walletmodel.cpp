...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fForceCheckBalanceChanged || nBestHeight != cachedNumBlocks || nPrivateSendRounds != cachedPrivateSendRounds || cachedTxLocks != nCompleteTXLocks)
    {
        fForceCheckBalanceChanged = false;
...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CRupeecoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CRupeecoinAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        if(recipients[0].useInstandSend && total > GetSporkValue(SPORK_5_MAX_VALUE)*COIN){
            return IXTransactionCreationFailed;
        }

        int nChangePos;
        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, nChangePos, strFailReason, coinControl, recipients[0].inputType, recipients[0].useInstandSend);
        transaction.setTransactionFee(nFeeRequired);

        if(recipients[0].useInstandSend && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE)*COIN){
            return IXTransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CRupeecoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
        ...
        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePos, strFailReason, coinControl, recipients[0].inputType, recipients[0].useInstandSend);
        transaction.setTransactionFee(nFeeRequired);
        ...
        if(!wallet->CommitTransaction(wtx, keyChange, (recipients[0].useInstandSend) ? "txlreq" : "tx"))
        {
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CRupeecoinAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CRupeecoinAddress(address).ToString());
...
{
    // Rupee context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CRupeecoinAddress(address).ToString())].push_back(out);
    }
}
