...
void ProcessMessageInstandSend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if(fLiteMode) return; //disable all privatesend/masternode related functionality
    if(!IsSporkActive(SPORK_2_INSTANDSEND)) return;
    if(!privateSendPool.IsBlockchainSynced()) return;

    if (strCommand == "txlreq")
    {
        //LogPrintf("ProcessMessageInstandSend::txlreq\n");
        ...
        BOOST_FOREACH(const CTxOut o, tx.vout){
            if(!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()){
                LogPrintf("ProcessMessageInstandSend::txlreq - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted)
        {
            ...
            LogPrintf("ProcessMessageInstandSend::txlreq - Transaction Lock Request: %s %s : accepted %s\n",
            ...
            LogPrintf("ProcessMessageInstandSend::txlreq - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str()
            );
            ...
            if (i != mapTxLocks.end()){
                ...
                if((*i).second.CountSignatures() >= INSTANDSEND_SIGNATURES_REQUIRED){
                    if(!CheckForConflictingLocks(tx)){
                        LogPrintf("ProcessMessageInstandSend::txlreq - Found Existing Complete IX Lock\n");
    ...
    else if (strCommand == "txlvote") //InstandSend Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
            ...
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                ...
                if(mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinMasternode.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstandSend::txlreq - masternode is spamming transaction votes: %s %s\n",
...
bool IsIXTXValid(const CTransaction& txCollateral){
    ...
    if(nValueOut > GetSporkValue(SPORK_5_MAX_VALUE)*COIN){
        LogPrint("instandsend", "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if(missingTx){
        LogPrint("instandsend", "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if(nValueIn-nValueOut < COIN*0.01) {
        LogPrint("instandsend", "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut-nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())){
        ...
        LogPrint("instandsend", "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_INSTANDSEND_PROTO_VERSION);

    if(n == -1)
    {
        LogPrint("instandsend", "InstandSend::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if(n > INSTANDSEND_SIGNATURES_TOTAL)
    {
        LogPrint("instandsend", "InstandSend::DoConsensusVote - Masternode not in the top %d (%d)\n", INSTANDSEND_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("instandsend", "InstandSend::DoConsensusVote - In the top %d (%d)\n", INSTANDSEND_SIGNATURES_TOTAL, n);
    ...
    if(!ctx.Sign()){
        LogPrintf("InstandSend::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if(!ctx.SignatureValid()) {
        LogPrintf("InstandSend::DoConsensusVote - Signature invalid\n");
        return;
    }
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_INSTANDSEND_PROTO_VERSION);

    CMasternode* pmn = mnodeman.Find(ctx.vinMasternode);
    if(pmn != NULL)
    {
        LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);
    }

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Unknown Masternode\n");
    ...
    if(n > INSTANDSEND_SIGNATURES_TOTAL)
    {
        LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", INSTANDSEND_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if(!ctx.SignatureValid()) {
        LogPrintf("InstandSend::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)){
        LogPrintf("InstandSend::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    }
    ...
    if (i != mapTxLocks.end()){
        ...
        LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if((*i).second.CountSignatures() >= INSTANDSEND_SIGNATURES_REQUIRED){
            LogPrint("instandsend", "InstandSend::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH(const CTxIn& in, tx.vin){
        if(mapLockedInputs.count(in.prevout)){
            if(mapLockedInputs[in.prevout] != tx.GetHash()){
                LogPrintf("InstandSend::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
bool CConsensusVote::SignatureValid()
{
    ...
    if(pmn == NULL)
    {
        LogPrintf("InstandSend::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if(!privateSendSigner.VerifyMessage(pmn->pubkey2, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("InstandSend::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    ...
    {
        int n = mnodeman.GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_INSTANDSEND_PROTO_VERSION);
