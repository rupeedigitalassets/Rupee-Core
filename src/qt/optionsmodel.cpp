...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRupeeAmount"))
        settings.setValue("nAnonymizeRupeeAmount", 1000);
    nAnonymizeRupeeAmount = settings.value("nAnonymizeRupeeAmount").toLongLong();
    if (settings.contains("nPrivateSendRounds"))
        SoftSetArg("-privatesendrounds", settings.value("nPrivateSendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRupeeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeRupeeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRupeeAmount:
            return QVariant(nAnonymizeRupeeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeRupeeAmount:
            nAnonymizeRupeeAmount = value.toInt();
            settings.setValue("nAnonymizeRupeeAmount", nAnonymizeRupeeAmount);
            emit AnonymizeRupeeAmountChanged(nAnonymizeRupeeAmount);
            break;
        default:
            break;
        }
