...
CDarkSendSigner privateSendSigner;
    ...
    if (strCommand == "dsa") { //DarkSend Accept Into Pool
        ...
        if(sessionUsers == 0) {
            if(pmn->nLastDsq != 0 &&
                ...
                std::string strError = _("Last PrivateSend was too recent.");
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, strError);
                return;
            }
        }
        ...
        if(dsq.ready) {
            ...
            if(state == POOL_STATUS_QUEUE){
                LogPrintf("PrivateSend queue is ready - %s\n", addr.ToString().c_str());
        ...
        //check it like a transaction
        {
            ...
            if (nValueIn > PRIVATESEND_POOL_MAX) {
                ...
                error = _("Value more than PrivateSend pool maximum allows.");
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, error);
                return;
            }
...
bool CPrivateSendPool::SetCollateralAddress(std::string strAddress){
    CRupeecoinAddress address;
    if (!address.SetString(strAddress))
    {
        LogPrintf("CPrivateSendPool::SetCollateralAddress - Invalid DarkSend collateral address\n");
        return false;
    }
...
bool CPrivateSendPool::IsBlockchainSynced()
{
...
void CPrivateSendPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        LogPrintf("CPrivateSendPool::Check() -- IS MASTER -- TRANSMITTING DARKSEND\n");
        ...
        LogPrintf("CPrivateSendPool::Check() -- txHash %d \n", txHash);
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Rupee and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002RUP per transaction on average.
            */
            if(r <= 10)
            {
...
bool CPrivateSendPool::IsCollateralValid(const CTransaction& txCollateral){
...

    {
        ...
        if(!AcceptableInputs(mempool, txCollateral, true, NULL)){
            LogPrintf ("CPrivateSendPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CPrivateSendPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, std::string& error){
...
bool CPrivateSendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, std::string& error, int newSessionID){
    if(fMasterNode) return false;
...
bool CPrivateSendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node){
    ...
    LogPrintf("CPrivateSendPool::SignFinalTransaction %s\n", finalTransaction.ToString());
            ...
            if(mine >= 0){ //might have to do this one input at a time?
                ...
                for(unsigned int i = 0; i < finalTransaction.vout.size(); i++){
                    BOOST_FOREACH(const CTxOut& o, e.vout) {
                        ...
                        if(Ftx == Otx){
                            //if(fDebug) LogPrintf("CPrivateSendPool::SignFinalTransaction - foundOutputs = %d \n", foundOutputs);
                            foundOutputs++;
...
bool CPrivateSendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeRupeeAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
bool CPrivateSendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral,  std::string& strReason)
{
...
void CPrivateSendPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const std::string error)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void CPrivateSendPool::RelayCompletedTransaction(const int sessionID, const bool error, const std::string errorMessage)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadCheckPrivateSendPool()
{
    ...
    RenameThread("Rupee-privatesend");
    ...
    while (true)
    {
        ...
        if(privateSendPool.IsBlockchainSynced()) {
