...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    if(!privateSendPool.pSubmittedToMasternode) return;
    
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
        if(privateSendPool.pSubmittedToMasternode->addr == pnode->addr) continue;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsee") { //PrivateSend Election Entry
        ...
        CTxOut vout = CTxOut((GetMNCollateral(pindexBest->nHeight)-1)*COIN, privateSendPool.collateralPubKey);
