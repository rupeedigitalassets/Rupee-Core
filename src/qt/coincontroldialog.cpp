    ...
    model(0)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_PRIVATESEND_ROUNDS, 120);
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // IX Fee
        if(coinControl->useInstandSend) nFee = max(nFee, CENT);
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CRupeecoinAddress(outputAddress).ToString());
            ...
            if(rounds >= 0) itemOutput->setText(COLUMN_PRIVATESEND_ROUNDS, strPad(QString::number(rounds), 15, " "));
            else itemOutput->setText(COLUMN_PRIVATESEND_ROUNDS, strPad(QString(tr("n/a")), 15, " "));
