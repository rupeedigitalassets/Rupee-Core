...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, bool privateSendMaster)
{
    if (pszDest == NULL) {
        ...
        if (pnode)
        {
            if(privateSendMaster)
                pnode->fPrivateSendMaster = true;
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Rupee " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rupee is probably already running."), addrBind.ToString());
        else
